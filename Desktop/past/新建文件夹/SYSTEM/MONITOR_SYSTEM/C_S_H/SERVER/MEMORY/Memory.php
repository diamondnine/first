...
<body>
	<div id="msg_TotalVisibleMemorySize" style="font-size:20px;margin-left:20px;margin-top:20px;"></div>
	<div id="msg_FreePhysicalMemory" style="font-size:20px;margin-left:20px;margin-top:5px;"></div>
		...
		window.setInterval(function(){
            $.ajax({
                ...
                success:function(data,textStatus){  
						...
						{	
						$("#msg_TotalVisibleMemorySize").text("总计空间量:"+data.Memory_TotalVisibleMemorySize+" KB");
						$("#msg_FreePhysicalMemory").text("剩余可用量:"+data.Memory_FreePhysicalMemory+" KB");
                        $("#msg_Usage").text("当前系统内存使用率:"+data.Memory_Usage+"%");
						
						var length = memory_usage.length;
						memory_usage.push(data.Memory_Usage);
						if(length>20)
						{
							for(var i=0;i<length-1;i++)
							memory_usage[i] = memory_usage[i+1];
							memory_usage.pop();
							memory_usage.length = 20;
						}	
						...
						location_context.arc(60+i*25,350-memory_usage[i]*3,2,0,360,false);
						location_context.fillStyle="red";//填充颜色,默认是黑色
						location_context.fill();//画实心圆
						location_context.closePath();
						}
						
						if(length>1)//获取的cpu使用率点个数多于一个就要画线
						{
							for(var i=0;i<length-1;i++)
							{
							location_context.moveTo(60+i*25,350-memory_usage[i]*3);
							location_context.lineTo(60+(i+1)*25,350-memory_usage[i+1]*3);
							}
						}
						...
						//开始绘制
						location_context.fillText("内存使用率",45,25);
